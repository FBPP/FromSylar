...
public:
	...
	int recv(iovec *buffers, size_t length, int flags = 0); //length iovec元素的个数
	int recvFrom(void *buffer, size_t length, Address::ptr from, int flags = 0);
	int recvFrom(iovec *buffers, size_t length, Address::ptr from, int flags = 0);


	Address::ptr getRemoteAddress(); //socket连的远端address
	Address::ptr getLocalAddress(); //本地用的ip
