...
LogEvent::LogEvent(std::shared_ptr<Logger> logger, LogLevel::Level level, const char *file, int32_t line, uint32_t elapse, uint32_t thread_id, uint32_t fiber_id, uint64_t time, const std::string &thread_name)
	: m_file(file), m_line(line), m_elapse(elapse), m_threadId(thread_id), m_fiberId(fiber_id), m_time(time), m_level(level), m_threadName(thread_name), m_logger(logger)
{	
...
void Logger::setFormatter(const std::string &val)
{
	...
	if(new_val->isError())
		{
			std::cout << "Loggset setFormatter name=" << m_name << "value = " << val << " invalid formatter" << std::endl;
			return;
		}
...
void FileLogAppender::reopen()
{
	...
	if(m_filestream)
		m_filestream.close();
	return m_filestream.open(m_filename);
}
	...
	for(size_t i= 0;  i < m_pattern.size(); ++i)
		{
			...
			while(n < m_pattern.size())
				{		
					...
					if(fmt_status == 1 && m_pattern[n] == '}')
						{
							fmt = m_pattern.substr(fmt_begin + 1, n - 1 - fmt_begin);
							fmt_status = 0;
							++n;
							break;
						}
			...
			if(fmt_status == 0)
			...
			else if(fmt_status == 1)
				{
					...
					vec.push_back(std::make_tuple("<<pattern_errno>>", fmt, 1));
					m_error = true;
				}		
