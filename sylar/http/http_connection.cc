						...
						, uint64_t timeout_ms)
{
	...
	if(!sock)
		return std::make_shared<HttpResult>((int)HttpResult::Error::CREATE_SOCKET_ERROR, nullptr, "connect fail:" + addr->toString());
	...
	if(!p)
		{
			...
			p = new HttpConnection(sock);
			++m_total;
		}
	return HttpConnection::ptr(p, std::bind(&ReleasePtr, std::placeholders::_1, this));
...
{
	++ptr->m_request;
	if(!ptr->isConnected() || ptr->m_createTime + pool->m_maxAliveTime >= GetCurrentMS() || ptr->m_request >= pool->m_maxRequest)
		{
			delete ptr;
			--pool->m_total;
			return;
					...
					, uint64_t timeout_ms)
{
	...
	if(rt == 0)
		return std::make_shared<HttpResult>(
				(int)HttpResult::Error::SEND_CLOSE_BY_PEER, nullptr, "send request closed by peer:" + sock->getRemoteAddress()->toString());
	if(rt < 0)
		return std::make_shared<HttpResult>(
				(int)HttpResult::Error::SEND_SOCKET_ERROR, nullptr,
				"send request socket error errno:" + std::to_string(errno) + "errstr=" + std::string(strerror(errno)));
